...
def task0():
     print ("Autumn Jacob score: 60/60")
...
def string_manip(s):
    spaces = s.strip()
    capital = spaces.upper()
    replacement = capital.replace(' ', '#')
    SI_replacement = replacement.replace('UMSI','')
    if len(SI_replacement) == 1:
        return SI_replacement
    else:
        return SI_replacement[::-1]
...
def name_counts(names):
    ...
    beglist = new_dict.items()
    seclist = sorted(beglist, key = lambda x: x[0]) 
    finlist = sorted(seclist, key = lambda x: x[1], reverse = True)
    return finlist  
